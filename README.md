...
# As we are manipulating objects, we need to close the file to prevent the data being modified which may occur when doing fittings.
photonPt_1.SetDirectory(0) # Avoid losing data after closing the file
...
# If the user wants to fit multiple functions to a set of data, I think it's a good idea to copy the original TH1 first. To begin, if more fits are to be performed later, the first fitted result will be deleted by default. Second, when plotting, the fitting result will always be on top of the TH1 data.
